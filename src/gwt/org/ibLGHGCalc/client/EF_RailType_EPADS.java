 ...
 */
public class EF_RailType_EPADS extends RestDataSource {
  private static EF_RailType_EPADS instance;

  public static EF_RailType_EPADS getInstance() {
    if (instance == null) {
      instance = new EF_RailType_EPADS();
    }
    return instance;
  }

  private EF_RailType_EPADS() {
    //set id + general stuff
    setID("eF_RailType_EPADS");
    setClientOnly(false);
    //setup fields
    ...
    setFields(idField, railTypeField, cO2MultiplierField,cO2MultiplierUnitField, cH4MultiplierField,cH4MultiplierUnitField, n2OMultiplierField, n2OMultiplierUnitField);
    //setup operations
    //1. fetch
    OperationBinding fetch =
        new OperationBinding(DSOperationType.FETCH,"EF_RailType_EPA/list");
    fetch.setDataProtocol(DSProtocol.POSTPARAMS);
    //2. update
    OperationBinding update =
        new OperationBinding(DSOperationType.UPDATE,"EF_RailType_EPA/save");
    update.setDataProtocol(DSProtocol.POSTPARAMS);
    //3. add
    OperationBinding add =
        new OperationBinding(DSOperationType.ADD,"EF_RailType_EPA/save");
    add.setDataProtocol(DSProtocol.POSTPARAMS);
    //4. remove
    OperationBinding remove =
        new OperationBinding(DSOperationType.REMOVE,"EF_RailType_EPA/remove");
