 ...
 */
public class EF_VehicleType_EPADS extends RestDataSource {
  private static EF_VehicleType_EPADS instance;

  public static EF_VehicleType_EPADS getInstance() {
    if (instance == null) {
      instance = new EF_VehicleType_EPADS();
    }
    return instance;
  }

  private EF_VehicleType_EPADS() {
    //set id + general stuff
    setID("eF_VehicleType_EPADS");
    setClientOnly(false);
    //setup fields
    ...
    OperationBinding fetch =
        new OperationBinding(DSOperationType.FETCH,"EF_VehicleType_EPA/list");
    fetch.setDataProtocol(DSProtocol.POSTPARAMS);
    //2. update
    OperationBinding update =
        new OperationBinding(DSOperationType.UPDATE,"EF_VehicleType_EPA/save");
    update.setDataProtocol(DSProtocol.POSTPARAMS);
    //3. add
    OperationBinding add =
        new OperationBinding(DSOperationType.ADD,"EF_VehicleType_EPA/save");
    add.setDataProtocol(DSProtocol.POSTPARAMS);
    //4. remove
    OperationBinding remove =
        new OperationBinding(DSOperationType.REMOVE,"EF_VehicleType_EPA/remove");
