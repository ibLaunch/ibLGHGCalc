 ...
 */
public class EF_PurchasedSteam_EPADS extends RestDataSource {
  private static EF_PurchasedSteam_EPADS instance;

  public static EF_PurchasedSteam_EPADS getInstance() {
    if (instance == null) {
      instance = new EF_PurchasedSteam_EPADS();
    }
    return instance;
  }

  private EF_PurchasedSteam_EPADS() {
    //set id + general stuff
    setID("eF_PurchasedSteam_EPADS");
    setClientOnly(false);
    //setup fields
    ...
    setFields(idField, fuelTypeField, cO2MultiplierField,cO2MultiplierUnitField, cH4MultiplierField,cH4MultiplierUnitField, n2OMultiplierField, n2OMultiplierUnitField);
    //setup operations
    //1. fetch
    OperationBinding fetch =
        new OperationBinding(DSOperationType.FETCH,"EF_PurchasedSteam_EPA/list");
    fetch.setDataProtocol(DSProtocol.POSTPARAMS);
    //2. update
    OperationBinding update =
        new OperationBinding(DSOperationType.UPDATE,"EF_PurchasedSteam_EPA/save");
    update.setDataProtocol(DSProtocol.POSTPARAMS);
    //3. add
    OperationBinding add =
        new OperationBinding(DSOperationType.ADD,"EF_PurchasedSteam_EPA/save");
    add.setDataProtocol(DSProtocol.POSTPARAMS);
    //4. remove
    OperationBinding remove =
        new OperationBinding(DSOperationType.REMOVE,"EF_PurchasedSteam_EPA/remove");
