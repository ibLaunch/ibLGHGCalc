 ...
 */
public class EF_AirTravelType_EPADS extends RestDataSource {
  private static EF_AirTravelType_EPADS instance;

  public static EF_AirTravelType_EPADS getInstance() {
    if (instance == null) {
      instance = new EF_AirTravelType_EPADS();
    }
    return instance;
  }

  private EF_AirTravelType_EPADS() {
    //set id + general stuff
    setID("eF_AirTravelType_EPADS");
    setClientOnly(false);
    //setup fields
    ...
    setFields(idField, airTravelTypeField, cO2MultiplierField,cO2MultiplierUnitField, cH4MultiplierField,cH4MultiplierUnitField, n2OMultiplierField, n2OMultiplierUnitField);
    //setup operations
    //1. fetch
    OperationBinding fetch =
        new OperationBinding(DSOperationType.FETCH, "EF_AirTravelType_EPA/list");
    fetch.setDataProtocol(DSProtocol.POSTPARAMS);
    //2. update
    OperationBinding update =
        new OperationBinding(DSOperationType.UPDATE,"EF_AirTravelType_EPA/save");
    update.setDataProtocol(DSProtocol.POSTPARAMS);
    //3. add
    OperationBinding add =
        new OperationBinding(DSOperationType.ADD,"EF_AirTravelType_EPA/save");
    add.setDataProtocol(DSProtocol.POSTPARAMS);
    //4. remove
    OperationBinding remove =
        new OperationBinding(DSOperationType.REMOVE,"EF_AirTravelType_EPA/remove");
